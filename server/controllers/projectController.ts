...
export const createProject = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const response: any = await ai.models.generateContent({
      model,
      contents: [imag1base64, imag2base64, prompt],
    ...
    if (!finalBuffer) {
      throw new Error("Failed to generaate image");
    }
    ...
    await prisma.project.update({
      where: { id: project.id },
      data: {
        generatedImages: uploadResult.secure_url,
        isGenerating: false,
      },
    });
    ...
    if (tempProjectId!) {
      // update project status amnd error message
      await prisma.project.update({
...
export const createVideo = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const image = await axios.get(project.generatedImages, {
    ...
    await prisma.project.update({
      where: { id: project.id },
      data: {
        generatedVideos: uploadResult.secure_url,
        isGenerating: false,
      },
    });
